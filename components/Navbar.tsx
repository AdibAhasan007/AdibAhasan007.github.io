  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/70 dark:bg-gray-950/60 backdrop-blur border-b border-slate-200 dark:border-slate-800">
      <nav className="container-pad mx-auto max-w-6xl flex items-center justify-between py-3">
        <a href="#hero" className="font-bold text-lg">Adib<span className="text-brand-600">.</span></a>
        <ul className="hidden md:flex items-center gap-6">
          {links.map(l => <li key={l.href}><a className="hover:text-brand-600" href={l.href}>{l.name}</a></li>)}
          <ThemeToggle />
        </ul>
          ...
          <>
            ...
            >
              ...
              <ul className="space-y-4">
                {links.map(l => (
                  <li key={l.href}><a className="hover:text-brand-600" href={l.href} onClick={()=>setOpen(false)}>{l.name}</a></li>
                ))}
              </ul>
              <div className="mt-auto pt-6"><ThemeToggle /></div>
            </motion.aside>
          </>
        )}
      </AnimatePresence>
